object testAlpha {
  ...
  class Sum(val operand1: Expression, val operand2: Expression) extends Expression {
   def execute =  {
	...
	};import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(1647); 
	
	
	// Testing
	
		var exp: Expression = Sum(Number(42), Sum(Number(3.14), Number(2.71)));System.out.println("""exp  : testAlpha.Expression = """ + $show(exp ));$skip(36); 
		println("value = " + exp.execute);$skip(50); 
		exp = Sum(Number(2), Sum(Number(3), Number(5)));$skip(36); 
		println("value = " + exp.execute);$skip(57); 
		exp = And(Boole(true), And(Boole(false), Boole(true)));$skip(36); 
