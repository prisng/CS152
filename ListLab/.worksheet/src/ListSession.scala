object listSession {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(85); 
	...
	def isPalindrome(n: String) = n == n.reverse;System.out.println("""isPalindrome: (n: String)Boolean""");$skip(287); 
  // ------------------------ //
	...
	cubeSumTR(List(3, 3, 3));System.out.println("""res8: Int = """ + $show(res$8));$skip(174); 
	
	// Map-filter-reduce
	def cubeSumMFR(vals: List[Int]) = {
		// filter -> odd numbers, map -> cube them, reduce -> sum them
		vals.filter(isOdd).map(cube).reduce(_ + _)
	};System.out.println("""cubeSumMFR: (vals: List[Int])Int""");$skip(251); 
	...
	**/
	...
	def makeOnes(n: Int): Stream[Int] = 1 #:: makeOnes(1);System.out.println("""makeOnes: (n: Int)Stream[Int]""");$skip(77); 
	
	// Testing the first 10 elements
	makeOnes(0).take(10).foreach(println _);$skip(100); 
	
	// B) Stream of all non-negative integers
	def nonNeg(n: Int): Stream[Int] = n #:: nonNeg(n + 1);System.out.println("""nonNeg: (n: Int)Stream[Int]""");$skip(75); 
	
	// Testing the first 10 elements
	nonNeg(0).take(10).foreach(println _);$skip(113); 
